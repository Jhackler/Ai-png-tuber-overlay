#!/usr/bin/env bash
# AS Adventurer (overlay) — portable Linux TUI launcher
...
# Open a real terminal when double-clicked from a file manager
open_in_terminal() {
...
maybe_placeholders() {
  [[ -f "$ROOT/generate-placeholders.js" ]] || return 0
  if compgen -G "$ROOT/public/assets/neutral_idle.*" >/dev/null 2>&1; then
    return 0
  fi
  if compgen -G "$ROOT/public/assets/*/neutral_idle.*" >/dev/null 2>&1; then
    return 0
  fi
  step "No character assets found — generating placeholders"
  "$NODE_BIN" "$ROOT/generate-placeholders.js" || warn "Placeholder generation skipped."
}
